...
| `cliente-guard-write.ps1` (stdin: evento PreToolUse) | Impide escribir identificadores de cliente en el **repo del plugin** (§10 de `docs/plugin-architecture.md`). Solo actúa dentro de ese repo — fuera, los nombres propios son legítimos. Capa 1 (lista `~/.claude/rs-clientes.json`, fuera del repo porque ahí sería la propia fuga) **bloquea**; capa 2 heurística (ruta de workspace de cliente, site Atlassian, `User Id=`, `"schema"`) solo **avisa**. Independiente del modo PII. `RS_CLIENTES_PATH` redirige la lista en tests |
