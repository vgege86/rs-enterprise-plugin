...
| Protección PII 🔷 | `/rs-pii [status\|bootstrap\|audit\|enforce\|off]`, "estado de protección PII", "activa/desactiva el enmascarado de datos personales en BD" | `rs-pii` (gestiona `pii_policy.mode` del modelo BD: `status` informa, `bootstrap` genera el inventario de columnas afectadas por muestreo real, `audit`/`enforce`/`off` cambian el modo; ⛔ gate de confirmación antes de escribir el modelo BD o `~/.claude/settings.json`, en cualquier dirección) |
