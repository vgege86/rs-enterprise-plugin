...
| `Invoke-Expression` | **ya no existe en el repo**: `runner/runner.ps1` lo perdió en v2.4.0 y `compile-check.ps1` / `test-runner-check.ps1` en versiones posteriores | Donde estaba era `dotnet build`/`dotnet test` sobre una cadena construida localmente. Hoy los tres usan el operador de llamada con array de argumentos (`& dotnet @dotnetArgs`), que no reinterpreta la cadena y cierra el vector de inyección vía rutas con comillas. |
