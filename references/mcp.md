...
| `parse_web_log(path, glob="*.log", desde="", niveles="ERROR,FATAL", max_signatures=30, samples=2)` | Parsea un log de errores web (NLog/log4net, ELMAH XML, volcado de stack .NET) y **agrupa por firma** (excepción + frame de código propio + mensaje normalizado) → `signatures[]` con `{hash, exception, origin, message, count, first_seen, last_seen, files, samples}` ordenado por `count`. Devuelve **solo el agregado**, nunca el log completo, y redacta PII en mensajes y muestras. `path` = fichero o carpeta. Base de `/rs-log-errores` |
