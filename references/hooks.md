...
|--------|-----------|-------------|
| `hooks/compile-check.ps1` | `<sln> [-NoRestore] [-Builder auto\|dotnet\|msbuild]` | Build real → `errors[], warnings[], success`, más `builder`/`builder_reason`/`projects`. **Autodetecta el compilador** vía `lib-msbuild.ps1` (MSBuild de VS con proyectos .NET Framework/web/COM, CLI `dotnet` con SDK-style modernos): llamar siempre a `dotnet` devolvía `MSB4019` sobre código correcto. ⛔ `builder_error` = el compilador necesario no está instalado → compilación **no verificada**, NO fallo del código. Parsea cualquier código `[A-Za-z]+\d+` (no solo `CS####`): un `MSB4019` real daba `error_count=0` con `exit_code=1`. Fija `DOTNET_CLI_UI_LANGUAGE=en`/`VSLANG=1033`: con el CLI localizado la severidad sale traducida (`advertencia CS0168`) y los warnings desaparecían del JSON sin error |
| `hooks/lib-msbuild.ps1` | (librería, no se invoca sola) | Decide con qué compilador construir una `.sln` leyendo lo que declara cada `.csproj`/`.vbproj` — **nunca por nombre de solución o de proyecto**, que en un plugin genérico envejece mal. `Get-RsBuildToolchain` → `@{builder; builder_path; requires_msbuild; reason; forced; projects; error}`; exige MSBuild de VS si algún proyecto es legacy (no SDK-style), tiene TFM `net4x`/`v4.x`, es web (`Microsoft.WebApplication.targets`) o lleva `COMReference`. Ante la duda, MSBuild: compila también los SDK-style, mientras que `dotnet` no compila .NET Framework. `Find-RsMsBuild`/`Find-RsVsTestConsole` (vswhere con `-products *`, para que valga una máquina con solo Build Tools) y `Get-RsTestAssembly` (el `.dll` de test más reciente de `bin\`, que es lo que come `vstest.console`). ⛔ Si el compilador necesario falta, devuelve `error` — quien llama debe **fallar cerrado**, no caer al otro en silencio. Gate: `tests/MsBuild.Tests.ps1` |
| `hooks/test-runner-check.ps1` | `<sln> [-NoBuild]` | Ejecuta los tests → `total/passed/failed/skipped`, `failures[]`, `source` (`trx`/`console`/`none`), `runner`. **Autodetecta el runner** con el mismo criterio que `compile-check.ps1` (`lib-msbuild.ps1`): `dotnet test` en soluciones SDK-style modernas, MSBuild + `vstest.console.exe` sobre el `.dll` compilado si hay proyectos .NET Framework, donde `dotnet test` no ejecuta ni una prueba. Lee el **.trx**, no el texto de consola. ⛔ Nunca devuelve verde sin evidencia: sin resumen legible → `parse_failed=true`+`success=false`; con 0 pruebas ejecutadas → `no_tests_ran=true`+`success=false`; sin `vstest.console` instalado → `runner_error` (**no ejecutados**, no rojos) |
...
| `hooks/installer-scripts.ps1` | `<workspace> <destino> [-Solo todo\|ddl\|objetos\|inserts] [-Tablas <lista ;-sep>] [-Conexion <id>] [-SinPlsql]` | Llama a `scripts/installer-tablas.py` + `installer-objects.py` + `installer-inserts.py` + `installer-gate-fuga.py` → `<destino>\Scripts`. ⛔ **La etapa `ddl` necesita ahora conexión a BD**: el DDL sale de la BD viva y no del `model.json` (`-Solo ddl` ya no funciona sin BD delante). Un exit ≠ 0 de `installer-tablas.py` **corta** — no ha escrito el fichero, así que seguir dejaría el paquete sin DDL de tablas. Al cerrar la etapa corre el **gate de fuga**: si algún artefacto lleva descripciones del modelo, marcas pii/safe o tickets internos, exit 1 y el paquete no es entregable. El inventario final lista los **seis** ficheros de objetos y marca `AUSENTE` el que falte (exit 2): antes era un comodín y, si no se generaba ninguno, el resumen salía OK igual. Resuelve la config de BD **y la visibilidad del esquema** una sola vez, y las pasa por `RS_DB_CONFIG_JSON` (sin password) y `RS_DB_VISIBILIDAD_JSON`. `-Conexion` se propaga a **todos** los scripts Python: si solo llegara a uno, el paquete mezclaría objetos leídos como dueño con inserts leídos con la cuenta de consulta. ⛔ Con 0 grants EXECUTE en Oracle y cuenta no dueña, `installer-objects.py` sale con **exit 1** y el hook corta — antes el paquete se generaba con los procedimientos vacíos y sin un solo aviso; `-SinPlsql` es la confirmación explícita de que el esquema de verdad no tiene PL/SQL. `-Solo`/`-Tablas` regeneran una parte o unas tablas concretas tras un fallo puntual (`-Tablas` implica `-Solo inserts` y **no** reescribe `Inserts\_run_all.sql`) |
...
Scripts Python asociados: `scripts/installer-tablas.py` (DDL de tablas sin schema, **desde la BD
viva**: columnas con tipo y tamaño exactos, NOT NULL, DEFAULT, IDENTITY, PK, UNIQUE, CHECK,
índices y FK — las FK al final, cuando ya existen todas las tablas),
`scripts/installer-gate-fuga.py` (falla la entrega si el paquete lleva descripciones del modelo,
marcas pii/safe o tickets), `scripts/installer-ddl.py` (**retirado**: shim que se niega y explica
que el DDL ya no sale del modelo),
...
| `hooks/find-symbol.ps1` | `<nombre> "<scope_dirs>" [-Type class\|method\|property\|interface\|enum\|any]` · o `-Symbols "A,B,C" -ScopeDirs "<dirs>"` | Localiza símbolo → `archivo:línea`. Con `-Symbols`, N símbolos en **una** pasada (salida `{symbols{}, total_symbols}`, una entrada por símbolo) — es lo que usa `batch_find_symbols`, que antes lanzaba un proceso y un recorrido del árbol por símbolo. `found` es un entero en modo simple y un booleano dentro de cada símbolo del lote; `matches` es **siempre** un array, también con una sola coincidencia |
...
| Script | Parámetros | Descripción |
|--------|-----------|-------------|
| `hooks/get-config.ps1` | `<workspace> [-Conexion <id>]` | Lee .rs-databases.json → `conexion, motor, datasource, schema, model_path` (de la conexión pedida, o la principal) + `conexiones[], motores[]` |
...
| `hooks/lib-buscar.ps1` | (librería, no se invoca sola) | Motor de búsqueda de texto en el árbol, compartido por `find-symbol.ps1`, `search-code.ps1` y `security-scan.ps1`. `Get-RsFicherosDeScope` enumera con `[IO.Directory]::EnumerateFiles` excluyendo carpetas (`bin`, `obj`, …); `Invoke-RsBusqueda` busca N patrones con **`Select-String`** y devuelve `{file, line, texto, patron}`, **una entrada por línea** con el primer patrón del array que casa. Quien necesite todos los patrones que casan una línea (`security-scan.ps1`, un hallazgo por regla) llama una vez por patrón reutilizando `-Ficheros`. ⛔ No usa ripgrep: `rg` existe como función del shell de la herramienta Bash, no como `rg.exe` en el PATH, y los hooks PowerShell no lo alcanzan. Suite: `tests/Search.Tests.ps1` |
...
| `hooks/lib-dbconfig.ps1` | (librería, no se invoca sola) | Lectura/validación de `.rs-databases.json` y parseo de cadenas de conexión — usada internamente por `get-config.ps1`, `db-query.ps1`, `check-env.ps1` y todos los hooks de sync. Dot-sourcea `lib-crypto.ps1` para descifrar el password. `Select-RsConexion` es el **único** sitio que resuelve el parámetro `-Conexion`, con sus tres guardarraíles: sin id → siempre `conexiones[0]` (nunca se infiere otra); id inexistente → `$null`, el llamante corta con la lista de válidas y **jamás** cae a `conexiones[0]`; y el id elegido se publica en la salida del llamante (sin eso, una lectura con cuenta privilegiada es indistinguible de una con la de consulta) |
| `hooks/lib-modeljson.ps1` | (librería, no se invoca sola) | `Save-RsModelJson` — la **única** forma de escribir `model.json` desde PowerShell. **No serializa**: transporta con `ConvertTo-Json -Compress` y delega en `scripts/_modeljson.py`, que impone el formato canónico (`indent=2`, `ensure_ascii`, CRLF, UTF-8 **con** BOM), escribe atómico y verifica antes de sustituir. ⛔ No usar `ConvertTo-Json` directamente sobre el modelo: el de PS 5.1 alinea cada valor a la columna de la clave padre (x3,2 de tamaño, diff inservible línea a línea) aunque el BOM y los CRLF sean correctos. `Test-RsModelJson` verifica un modelo ya escrito sin tocarlo |
| `hooks/lib-dbvisibilidad.ps1` | (librería, no se invoca sola) | Distingue "el objeto no existe" de "mi cuenta no lo ve" — Oracle no da forma de hacerlo (ORA-00942 es ambiguo y `ALL_*` está filtrada por privilegio), así que la regla es **no concluir la ausencia**. `Get-RsVisibilidad`: es-dueña, GRANTs por privilegio y conteos de `ALL_OBJECTS` por tipo. `New-RsCobertura`/`Format-RsCobertura`/`Merge-RsCobertura`: contraste real-vs-capturado, con las exclusiones deliberadas declaradas aparte, volcado al `_cobertura` del modelo mezclando por sección. `Set-RsTablaVisible`: marca `visible:false` + `visible_check` **sin** borrar columnas ni índices. Solo Oracle (`soportado:false` en SQL Server → el llamante omite el bloque, no inventa semántica) |
| `hooks/db-visibilidad.ps1` | `<workspace> [-Conexion <id>]` | Hook ejecutable de `lib-dbvisibilidad.ps1` → `{ok, soportado, conexion, usuario, esquema, es_dueno, grants{}, diccionario{}, tipos{}}`. Existe además de la librería para que los scripts Python usen la **misma** implementación por subproceso (cacheada en `RS_DB_VISIBILIDAD_JSON`) en vez de reescribir las consultas |
...
| `hooks/lib-trx.ps1` | (librería, no se invoca sola) | Lectura del resultado de `dotnet test` sin depender del idioma del CLI, usada por `test-runner-check.ps1`. `Get-TrxSummary`: agrega los `.trx` (XML → los conteos son atributos, no frases) y saca los fallos con mensaje aplanado; los **saltados** se toman como `max(notExecuted+inconclusive, total-executed)` porque VSTest deja `notExecuted="0"` aunque haya resultados `NotExecuted`. `Get-DotnetConsoleSummary`: último recurso, entiende los rótulos en inglés **y** en español, y marca `inconsistent` si las cifras no cuadran. `Get-RsLineasResumen`: líneas de evidencia (convierte los `ErrorRecord` de `2>&1` a texto, si no el JSON se llena del objeto entero). ⛔ Ambos parsers devuelven `$null` —nunca cero— cuando no pueden contar: el cero silencioso fue el bug de la 3.16.0 |
...
| `hooks/compare-model.ps1` | `<workspace> [-Tables "T1,T2"] [-Conexion <id>]` | Drift model.json vs esquema real BD. ⛔ "tabla eliminada" con una cuenta que solo ve por GRANT puede ser una tabla que existe y no se ve: contrastar con la cobertura de `sync-from-db.ps1` antes de borrar nada |
| `hooks/generate-migration.ps1` | `<workspace>` | Scripts SQL (CREATE TABLE / ALTER TABLE ADD) desde drift |
| `hooks/sync-model-objects.ps1` | `<workspace> [<proyecto>] [-DryRun] [-Conexion <id>]` | Sincroniza a `model.json` el **inventario de objetos de BD** (vistas, procedimientos, paquetes, funciones, triggers, sinónimos, secuencias): ficha + **firma** del cuerpo, nunca el cuerpo. Reutiliza los extractores de `installer-objects.py`, así que no duplica consultas y la firma se calcula sobre el mismo texto que emitiría el instalador. `-DryRun` lista el inventario y el diff sin escribir. exit 2 = **parcial**, por dos causas: un tipo falló (su sección se **conserva** del modelo anterior en vez de vaciarse) o hay **hueco de cobertura** (el diccionario ve más objetos de los capturados). Ni una ni otra se pueden leer como "esos objetos ya no existen" |
| `hooks/actualizador-objetos.ps1` | `<workspace> <destino\scripts> [<proyecto>] [-Prefijo NN] [-Nombre f.sql] [-Sincronizar] [-DryRun] [-Conexion <id>]` | Genera el `.sql` de los **objetos de BD que han cambiado desde la última entrega** para el paquete de `/rs-actualizador`. Línea base = inventario `objetos` del `model.json`; sin él no hay contra qué comparar y lo dice en vez de inventarlo. Emite el mismo texto que entregaría el instalador (`render_objeto`), en orden de dependencias, con prefijo en la franja 90-98 (tras los scripts de las tareas, antes del `99-RVERSIONES`). ⛔ Una **secuencia modificada NO viaja** (su DDL es `CREATE`/`DROP`+`CREATE`: reiniciaría el contador del cliente) y de lo **eliminado no se emite ningún `DROP` activo** — ambos salen listados. ⛔ Con **hueco de cobertura** (la cuenta no ve todo el esquema) el delta **no se escribe**: un tipo invisible se leería como "se ha eliminado todo". `-Sincronizar` avanza la línea base (solo con la entrega cerrada). exit 2 = generado pero con algún tipo sin extraer |
| `hooks/ddl-objeto.ps1` | `<workspace> <OBJETO> [<proyecto>] [-Seccion <sec>] [-Out <fichero>] [-Conexion <id>]` | DDL de **un** objeto leído de la BD viva. El modelo guarda ficha y firma, nunca el cuerpo: esto es lo que permite leer el código que vive dentro de la BD durante el desarrollo sin abrir un cliente de BD. Compara la firma de la BD con la del modelo y avisa si no coinciden. Sin `-Seccion` usa el inventario para saber dónde mirar; si el objeto no está en él, barre los siete tipos |
| `hooks/sync-from-db.ps1` | `<workspace> <proyecto> [-Conexion <id>]` | Sincroniza tablas/columnas desde BD real → `table_count`, `tablas_leidas`, `no_visibles[]`, `cobertura`, `defaults` (escritura canónica atómica). Captura el **valor DEFAULT** de cada columna (campo `default`) y la **posición real dentro de la PK** (campo `pk` como ordinal), y **preserva `description` y las marcas manuales `pii`/`safe`** (vía `lib-dbmodel.ps1`). ⛔ Una tabla del modelo que no salga en la lectura **no se borra**: se conserva entera y se marca `visible:false`. exit 2 / `parcial:true` cuando hay hueco |
| `hooks/sync-model-tables.ps1` | `<workspace> <tablas-coma-separadas> [-Conexion <id>]` | Actualiza tablas específicas de model.json post-migración, incluidos los `default` de columna y la posición dentro de la PK, y preservando `description`/`pii`/`safe`. `not_in_db[]` son las tablas que no se leyeron — que con una cuenta sin GRANT puede significar "no existe" o "no la veo"; ninguna se toca |
| `hooks/sync-indexes.ps1` | `<workspace> [-Proyecto <nombre>] [-Conexion <id>]` | Sincroniza índices desde BD al modelo — preserva source=manual. ⛔ **Solo toca las tablas que la lectura ve**: antes recorría todas las del modelo y reescribía `indexes` con lo que hubiera, así que una tabla sin GRANT se quedaba con **0 índices** y el modelo salía peor de como entró, en silencio. Devuelve `tablas_intactas` y `cobertura`; exit 2 / `parcial:true` con hueco |
...
| `hooks/cliente-guard-write.ps1` | (stdin: evento PreToolUse) | Guarda PreToolUse sobre `Write`/`Edit`: impide escribir identificadores de cliente en el **repo del plugin** (regla de `docs/plugin-architecture.md` §10). ⛔ **Solo actúa dentro del repo del plugin** — sube desde la ruta destino buscando un `.claude-plugin/plugin.json` con `"name": "rs-enterprise-agent"`; fuera sale por 0, porque en el workspace del cliente los nombres propios son legítimos. **Capa 1** (lista de `~/.claude/rs-clientes.json`: `nombres`, `dominios`, `esquemas`; tokens de <3 caracteres se ignoran) → **bloquea**; casa en el contenido **y en la ruta**, insensible a mayúsculas, con frontera `\w` (el guion no cuenta: `informe-<cliente>.md` sí casa, `<cliente>ter` no). **Capa 2** heurística (rutas `X:\SVN\|GIT\RS\<segmento>\`, `https://<x>.atlassian.net`, `User Id=`, `"schema"`) → **solo avisa**. Sin fichero de lista, la capa 1 queda inactiva; lista ilegible → avisa y no bloquea. **No** depende del modo PII del workspace. Se excluye a sí misma y a `tests/ClienteGuard.Tests.ps1` de la capa 2 (son los únicos ficheros que llevan los patrones a propósito). Override para tests: `RS_CLIENTES_PATH` |
...
| `scripts/check-requisitos.ps1` | `[-Quiet] [-Reparar] [-Force]` | Comprueba los requisitos del **propio servidor MCP**: `python` en el PATH (≥3.11 y no el marcador de la Microsoft Store) y el paquete `mcp` con `mcp.server.fastmcp` (línea 1.x). ⛔ **No es fallback de ninguna tool MCP** — es lo contrario: diagnostica que el MCP no puede arrancar, y por eso es PowerShell puro. Registrado en `SessionStart` (`-Quiet`: calla si todo está bien, con marcador `~/.claude/.rs-requisitos-ok` que lleva la versión del plugin, así que un arranque normal no spawnea Python). `-Reparar` instala `mcp>=1.2.0,<2` con el pip de ese Python — solo tras confirmación explícita del usuario; `-Force` ignora el marcador. Sale por 0 siempre: informa, nunca bloquea la sesión. Lo usa `rs-validar-entorno` como paso 0 de `/rs-env` |
| `scripts/verificar-sync.ps1` | `[-Fuente <ruta>] [-Cache <ruta>] [-Quiet]` | Comprueba que la copia del plugin que se **ejecuta** (`~\.claude\plugins\cache\rs-enterprise-agent\rs-enterprise-agent\<version>\`) coincide con la fuente de la que dice venir (`~\.claude\plugins\marketplaces\rs-enterprise-agent`). Primero la `version` de `plugin.json`; si coincide, fichero a fichero por SHA-256 normalizando el fin de línea (git puede materializar CRLF en un árbol y LF en el otro). Distingue tres casos, que se arreglan distinto: contenido distinto · en la fuente y sin ejecutarse · **ejecutándose y ausente de la fuente** (editado en el cache: fuera de git y condenado a desaparecer al actualizar). ⛔ **No copia nada** — sincronizar es de `/plugin marketplace update`, y hacerlo por debajo dejaría el cache en un estado que Claude Code no sabe que existe. Registrado en `SessionStart` con `-Quiet` (calla si todo cuadra, exit 0 siempre); sin `-Quiet` devuelve 1 si hay desajuste, para encadenarlo en el paso 9 de `rs-plugin-dev`. Ver CHANGELOG 3.25.0 |
...
| `hooks/parse-weblog.ps1` | `-Path <fichero\|carpeta> [-Glob *.log] [-Desde <ISO>] [-Niveles ERROR,FATAL] [-MaxSignatures 30] [-Samples 2] [-MaxLines 500000]` | Parsea logs de error web (NLog/log4net, ELMAH XML, **`rs-cerrores`** —el formato propio de la AgendaWeb— y stack .NET plano) y agrupa por **firma** (`SHA1` de excepción + frame de código propio + mensaje normalizado) → `signatures[]` con recuento, ventana temporal, `pantalla` y muestras. En `rs-cerrores` la "excepción" es el **código** (`ORA-xxxxx` → `Codigo error: <n>` → excepción .NET) y el stamp se normaliza a `yyyy-MM-dd HH:mm:ss`. Emite **solo el agregado** (el log nunca entra en contexto) y pasa mensajes/muestras por `Remove-RsPii` **más** la redacción de literales SQL (`'…'` → `'<val>'`), donde viaja el dato que no tiene forma reconocible. Detecta la codificación del log (ANSI vs UTF-8). Fallback 1:1 de `parse_web_log`; base de `/rs-log-errores`. Cubierto por `tests/WebLogParser.Tests.ps1` |
