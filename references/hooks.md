...
|--------|-----------|-------------|
| `hooks/compile-check.ps1` | `<sln> [-NoRestore]` | Build real → `errors[], warnings[], success`. Fija `DOTNET_CLI_UI_LANGUAGE=en`/`VSLANG=1033` antes de invocar a dotnet: con el CLI localizado la severidad sale traducida (`advertencia CS0168`) y los warnings desaparecían del JSON sin error |
| `hooks/test-runner-check.ps1` | `<sln> [-NoBuild]` | dotnet test → `total/passed/failed/skipped`, `failures[]`, `source` (`trx`/`console`/`none`). Lee el **.trx** (`--logger trx`), no el texto de consola. ⛔ Nunca devuelve verde sin evidencia: sin resumen legible → `parse_failed=true`+`success=false`; con 0 pruebas ejecutadas → `no_tests_ran=true`+`success=false` |
