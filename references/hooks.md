...
| `hooks/sync-from-db.ps1` | `<workspace> <proyecto>` | Sincroniza tablas/columnas desde BD real → `table_count`, `defaults` (escritura atómica). Captura el **valor DEFAULT** de cada columna (campo `default`) y **preserva `description` y las marcas manuales `pii`/`safe`** (vía `lib-dbmodel.ps1`), que antes se borraban en cada sync |
